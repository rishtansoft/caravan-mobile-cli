...
import { MainPhoneUpdateSmcCodeProps } from './RouterType';
...
const VerifySmsScreen: React.FC<MainPhoneUpdateSmcCodeProps> = ({ navigation }) => {
    ...
    useEffect(() => {
        if (code.every(digit => digit !== '')) {
            axios.post(API_URL + '/api/auth/verify-update-phone', {
                "user_id": user_id,
                "sms_code": code.join(''),
                "new_phone": '+' + phone
            }, {
    ...
    const resetTimer = () => {
        if (token && user_id && phone) {
            axios.post(API_URL + '/api/auth/resend-code-update-phone', {
                "user_id": user_id,
                "new_phone": '+' + phone
            }, {
                headers: {
        ...
        <View style={styles.container}>
            <View style={styles.header_con}>
                <View style={{
                    ...
                    <Icon onPress={() => navigation.navigate('main_phone_update')} name="angle-left" size={30} color="#7257FF" />
                </View>
